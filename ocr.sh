...
in_file_name_base=quiz_11-10

out_file_name_base=quiz_11-10
...
rm $in_file_name_base/ -r 2> /dev/null || true

mkdir $in_file_name_base -p
mkdir $in_file_name_base/student-pdfs -p
mkdir $in_file_name_base/student_page_lists -p
...
total_pages=$(pdftk "${in_file_name_base}.pdf" dump_data | grep NumberOfPages | awk '{print $2}')
...
for batch in $(seq 0 $(( (total_pages + convert_batch_size - 1) / convert_batch_size - 1 ))); do
    ...
    # Use ImageMagick's bracket notation for page ranges
    convert -density $pdf_rasterization_density \
            -crop "${name_width}x${name_height}+${name_x_offset}+${name_y_offset}" \
            "${in_file_name_base}.pdf[${start_page}-${end_page}]" \
            "${in_file_name_base}/page-%d.jpg"
done
...
# ╰───────────────────────────────────────────────╯
i=0
while [[ -e  $in_file_name_base/page-$i.jpg ]]; # If file exists.
do 
  # ⋘──────── Run tesseract OCR ────────⋙
  tesseract_in_file=$in_file_name_base/page-$i.jpg
  tesseract_out_file=$in_file_name_base/page-$i 
  student_names_file=student_names.user-words
  tesseract $tesseract_in_file $tesseract_out_file --user-words $student_names_file
  ...
  # * "[^a-zA-Z]+": White space and other characters, not including letters. Sometimes the OCR produces "_" due to the underlining.
  # * "\w+([ ]+\w+)*)": The student's name
  student=$(cat $in_file_name_base/page-$i.txt | sed -Ez 's/^.*Name:[^a-zA-Z]+([a-zA-Z.]+([ ]+\w+)*).*$/\1/')
  ...
  echo $(($i + 1)) >> "$in_file_name_base/student_page_lists/$student.txt"
...
# ╰─────────────────────────────────────────────────────────────────────╯
for filename in  $in_file_name_base/student_page_lists/*; do 
  ...
  pdftk $in_file_name_base.pdf cat $student_pages output "$in_file_name_base/student-pdfs/${out_file_name_base} $student.pdf"
done
